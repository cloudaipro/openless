...
///
/// **流式路径的繁简处理**（Feature 2d）：`apply_chinese_script_preference` 现在在
/// 每个 flush 缓冲（~12ms 聚合，通常含整词）粒度上跑字级 s2tw 转换后才落字，堵住
/// 「串流跳过 OpenCC、繁化只能靠 LLM」。
/// **仍不在流式路径里做**：`apply_correction_rules` 与中英混用整行正规化
/// （`cjk_postprocess::apply_code_switch`）—— 这些需要完整上下文 / 不好在已落字后回退，
/// 只在一次性路径（`finalize_polished_text`）做。需要时可关 streaming toggle 走一次性路径。
...
async fn run_streaming_polish(
    ...
    // 串流路徑的繁簡處理（Feature 2d）：對每個 flush 緩衝做字級簡繁轉換後才落字。
    // s2tw 是字級映射，在 12ms flush 邊界上基本安全且冪等（繁體字不在 s2t 源鍵裡，
    // 重複轉換不變）。中英混用的整行正規化需完整上下文，串流路徑不做（見 finalize_polished_text）。
    let script_pref = chinese_script_preference;
    let typer_handle = tokio::task::spawn_blocking(move || {
        drain_streaming_insert_deltas_with(
            rx,
            STREAMING_INSERT_FLUSH_INTERVAL,
            move |pending, typed| {
                if !pending.is_empty()
                    && !matches!(script_pref, crate::types::ChineseScriptPreference::Auto)
                {
                    *pending = apply_chinese_script_preference(pending, script_pref);
                }
                flush_streaming_insert_buffer(pending, typed)
            },
        )
    });
...
fn finalize_polished_text(
    ...
    // 中英混用（晶晶體）正規化：英文為主行去中文腔、全形英數→半形、合併拆字。
    // 只在一次性路徑做（需要整行上下文）；串流路徑只做字級簡繁轉換。
    let polished = super::cjk_postprocess::apply_code_switch(&polished);
...
pub(super) async fn begin_session_as(
    ...
    if is_bailian_provider(&active_asr) {
        ...
        // 用户辞書を MiMo の語彙ヒント（chat text パート）にも流す（Whisper と同じエントリ）。
        let mimo_prompt =
